...
const openAlertDetail = async (alert: AlertRecord) => {
  currentAlert.value = alert
  showDetailModal.value = true
  if (alert.status === 'unread') {
    await mockMarkAlertRead(alert.id)
    alert.status = 'read'
    fetchStats()
  }
  detailLoading.value = true
  try {
    alertHandleRecords.value = await mockGetAlertHandleRecords(alert.id)
  } finally {
...
const handleMarkRead = async (alert: AlertRecord) => {
  try {
    await mockMarkAlertRead(alert.id)
    fetchAlerts()
    fetchStats()
